...
class Tue : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val v = LayoutInflater.from(context).inflate(R.layout.fragment_tue, null)
        ...
        try {
            ...
            val tue = obj.getJSONObject("Tuesday")
            val recyclerView: RecyclerView = v.findViewById(R.id.rec_timetable)
            val list = ArrayList<Schedule>()
            val keysToCopyIterator: Iterator<*> = tue.keys()
            ...
            for (i in keysList.indices) {
                val morning9 = tue.getString(keysList[i])
