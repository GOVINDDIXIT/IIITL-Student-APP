...
class Mon : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val v = LayoutInflater.from(context).inflate(R.layout.fragment_mon, null)
        ...
        try {
            ...
            val monday = obj.getJSONObject("Monday")
            val list = ArrayList<Schedule>()
            val keysToCopyIterator: Iterator<*> = monday.keys()
            ...
            for (i in keysList.indices) {
                val morning9 = monday.getString(keysList[i])
